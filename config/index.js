...
class SLAMBOX {
    ...
    async rimrafLocal() {
        ...
        return fsPromises.rmdir(path.resolve(__dirname, '../local/templates/html'), { recursive: true })
    }
    ...
    server() {
        ...
        browserSync({
            ...
            files: [
                "./local/templates/html/components-template/**/style.css",
                ...
                "./local/templates/html/images/",
            ],
            middleware: [
                function(req, res, next) {
                    if (lightMode) {
                        if (req.originalUrl.match(/(?<=\/local\/templates\/html\/).*(?=.*\.html)/i) && !req.originalUrl.includes('ajax')) {
                            _this.currentPage = req.originalUrl.match(/(?<=\/local\/templates\/html\/).*(?=.*\.html)/i)[0];
                        }
                    };
                    next();
                },
        ...
        return new Promise((resolve, reject) => {
            ...
            glob("src/components/**/script.js", function(er, files) {
                files.forEach(src => {
                    ...
                    const output = path.resolve(__dirname, '../local/templates/html/components-template/', src.replace('src/components/', '').replace('script.js', 'script.min.js'));
                    promises.push(
                        buildJs({
                            input,
                            output,
