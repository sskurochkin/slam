...
pagesGlob.forEach(page => {
	...
	entry.forEach(script => {
		const pathToScript = path.resolve(__dirname, `../local/templates/html/${script}`);
		console.log('path', pathToScript);
		const res = fs.readFileSync(pathToScript, 'utf-8');
		scripts.push(res);
	})
	const folder = path.resolve(__dirname, `../local/templates/html`);
	const directory = fs.readdirSync(folder, 'utf-8');
	if (!directory.includes('combinedJS')) {
		fs.mkdirSync(`${folder}/combinedJS`);
	}
	fs.writeFileSync(`${folder}/combinedJS/${page.replace('local/templates/html/', '')}.js`, scripts.join(''))
	html = fs.readFileSync(page, 'utf-8');
	const replaceItem = html.match(/<script data="component"><\/script>/);
	const newHtml = html.replace(replaceItem, `<script src="./combinedJS/${page.replace('local/templates/html/', '')}.js" defer></script>`);
	fs.writeFileSync(page, newHtml);
	scripts = [];
})
